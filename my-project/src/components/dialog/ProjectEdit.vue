<template>
  <el-dialog :model-value="modelValue" width="550px" :show-close="false" :before-close="handleClose" >
    <template #title>
      <el-space>
        <span style="font-weight: bold">
          {{kanban.name}}
        </span>
        <el-tag class="ml-2" effect="dark">{{this.kanban.type}}</el-tag>
      </el-space>
    </template>
    <el-space>
      <el-descriptions class="margin-top" :column="2"  border>
        ...
        <el-descriptions-item span="2">
          ...
          <el-input v-model="kanban.name"></el-input>
        ...
        <el-descriptions-item span="1">
          ...
          <el-radio-group v-model="kanban.owner" size="small">
        ...
        <el-descriptions-item >
          ...
          <el-radio-group v-model="kanban.type" size="small">
...
<!--        </el-descriptions-item>-->
        ...
        <el-descriptions-item span="2">
          ...
          <el-space>
            <el-date-picker v-model="pickDate" type="date" placeholder="选择截止日期"></el-date-picker>
            <el-link type="primary" style="font-size: 6px" @click="setDeadline(1)">一天</el-link>
            <el-link type="primary" style="font-size: 6px" @click="setDeadline(3)">三天</el-link>
            <el-link type="primary" style="font-size: 6px" @click="setDeadline(10)">十天</el-link>
            <el-link type="primary" style="font-size: 6px" @click="setDeadline(30)">三十天</el-link>
          </el-space>
...
export default {
  ...
  methods: {
    cropperImgUpload(data) {
      this.kanban.imgPath = data.imagePath
    },
    handleClose(){
    ...
    editKanban() {
      this.$axios.put("/kanban/project", this.kanban).then(()=>{
        this.$emit('update:modelValue', false)
        this.reload()
      })
    },
  ...
  watch: {
    ...
    modelValue(newVal) {
      if (newVal===true) {
        this.$axios.get("/kanban/project/" + this.id).then((res) => {
